...
describe('auth routes - client-secret strategy', () => {
  beforeAll(async () => {
    vi.clearAllMocks()
  ...
  describe('GET /auth/sign-out-oidc', () => {
    ...
    test('redirects to signed out page if authenticated', async () => {
      const response = await server.inject({
        url: path,
        auth: {
          strategy: 'session',
    ...
    test('redirects to signed out page if not authenticated', async () => {
      const response = await server.inject({
        url: path
      })
      expect(response.statusCode).toBe(HTTP_STATUS_FOUND)
