...
describe('personal address check', () => {
  ...
  describe('POST /customer/{crn}/account-address-check', () => {
    ...
    describe('when a request is valid', () => {
      test('it calls updatePersonalAddressChangeService and redirects to personal-details', async () => {
        ...
        expect(h.redirect).toHaveBeenCalledWith('/customer/1234567890/personal-details')
      })
    })
    ...
    describe('when the crn fails validation', () => {
      ...
      test('it redirects to the search-crn page and does not update', async () => {
        await postPersonalAddressCheck.handler(request, h)

        expect(h.redirect).toHaveBeenCalledWith('/search-crn')
        expect(updatePersonalAddressChangeService).not.toHaveBeenCalled()
      })
    })
  })
})
