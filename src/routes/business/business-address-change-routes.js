...
const getBusinessAddressChange = {
  ...
  handler: async (request, h) => {
    ...
    const businessDetails = await fetchBusinessChangeService(yar, email, 'changeBusinessPostcode')
...
const postBusinessAddressChange = {
  ...
  options: {
    ...
    validate: {
      ...
      failAction: async (request, h, err) => {
        ...
        const pageData = await businessAddressChangeErrorService(yar, email, payload.postcode, err.details)
  ...
  handler: async (request, h) => {
    ...
    if (addresses.error) {
      const pageData = await businessAddressChangeErrorService(yar, email, payload.postcode, addresses.error)
