...
describe('businessAddressChangeErrorService', () => {
  ...
  describe('when preparing error page data', () => {
    test('it fetches business details with changeBusinessPostcode', async () => {
      ...
      await businessAddressChangeErrorService(yar, email, 'SW1A 1AA', errors)

      expect(mockFetchBusinessChangeService).toHaveBeenCalledWith(yar, email, 'changeBusinessPostcode')
    })

    test('it returns page data with postcode and errors', async () => {
      const errors = [{ message: 'Postcode not found' }]

      const result = await businessAddressChangeErrorService(yar, email, 'SW1A 1AA', errors)
  ...
  describe('when there are no validation errors', () => {
    test('it returns page data with empty errors', async () => {
      const result = await businessAddressChangeErrorService(yar, email, 'SW1A 1AA', [])
