...
const getSchemasToValidate = (hasUprn) => {
  ...
  if (!hasUprn) {
    schemasToValidate.push(schemas.business.details.address)
  }
...
const mapBusinessDetails = (businessDetails, hasUprn) => {
  ...
  if (!hasUprn) {
    ...
    flatBusinessDetails.county = businessDetails.address?.manual?.line4 ?? ''
    flatBusinessDetails.city = businessDetails.address?.city ?? ''
