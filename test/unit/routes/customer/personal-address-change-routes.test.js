  ...
  describe('GET /customer/{crn}/account-address-change', () => {
    describe('when a request is valid', () => {
      ...
      test('should render personal-address-change view with page data', async () => {
        ...
        expect(h.view).toHaveBeenCalledWith('personal/personal-address-change', expect.objectContaining({
    ...
    describe('when the crn fails validation', () => {
      test('the route has a pre-handler to validate crn', () => {
        expect(getPersonalAddressChange.options.pre).toBeDefined()
        expect(getPersonalAddressChange.options.pre).toHaveLength(1)
      })
    })
  })
